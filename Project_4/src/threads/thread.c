...
thread_set_nice (int nice) 
{
	...
	int temp = float_div_int(t->recent_cpu,4);
	temp /= FRAC;
  temp = PRI_MAX - temp - (2*t->nice);
  if(temp>PRI_MAX)
	  t->priority=PRI_MAX;
...
{
  return int_mul_float(100,load_avg)/FRAC;
}
...
{
	return int_mul_float(100,thread_current()->recent_cpu)/FRAC;
}
...
void update_load_avg_and_recent_cpu(void){
	...
	int temp = int_mul_float(59,load_avg);
	temp = int_plus_float(num_threads,temp);
	load_avg=float_div_int(temp,60);
	//load_avg update complete
	
	struct list_elem *e;
	for(e=list_begin(&all_list);e!=list_end(&all_list);e=list_next(e)){
		struct thread* t=list_entry(e,struct thread, allelem);
		if(t!=idle_thread){
			int temp1 = int_mul_float(2,load_avg);
			int temp2 = int_plus_float(1,temp1);
			temp1=float_div_float(temp1,temp2);
			temp1=float_mul_float(temp1,t->recent_cpu);
			temp1=int_plus_float(t->nice,temp1);
			t->recent_cpu=temp1;
		}
	}
	//recent_cpu of all thread updated
}
...
//needs to be updated every 4 ticks
...
#ifdef VM
long long unsigned return_time(void){
	long long temp=user_ticks;
	return temp;
}
#endif
